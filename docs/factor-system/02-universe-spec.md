...
| 과거 시점 지수 구성종목 | **지원** — Sharadar `sp500` 은 `current`/`historical`/`added`/`removed` 를 1957년부터 제공한다. 초판 문서가 무료 티어만 보고 "`current` 만 제공" 이라 적었으나 실측으로 뒤집혔다 |
| 상장폐지 종목 | **포함** — 현재 스토어에 20,931종목/1997~2026, 엔론(ENRNQ)·구 아메리칸항공(AAMRQ)·암박(ABKFQ) 실재 확인. 생존편향은 해소됐다 |
