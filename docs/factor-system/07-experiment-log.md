...
포트폴리오 구성 기법 중 미구현 항목이다. 값어치 순.

1. **비용 인지 최적화** — 목적함수가 비용을 모른다. `hold_multiple`(매매
   유예구간)을 구현했으나 아직 검증 전이다. 현재 회전율은 리밸런싱당
   67.2%, 연 2.7회전이며 수수료 0.5% 기준 **연 2.7%** 가 비용이다.
2. **변동성 타게팅** — 지금 타이밍은 켜짐/꺼짐 이진이다. 연속 익스포저로
   포트폴리오 변동성을 일정하게 유지하는 방식(Moreira & Muir 2017) 미적용.
3. **파라미터 앙상블** — 폴드마다 최적값 하나만 쓴다. `n_stocks` 가
   15↔47 로 널뛴 원인이다. 상위 k개 평균이 그 불안정성을 상쇄한다.
4. **섹터 제약** — 팩터는 섹터 중립화하나 최종 포트폴리오의 섹터 비중은
   보지 않는다. 20종목이 한 섹터에 몰릴 수 있다.
