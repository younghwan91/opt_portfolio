...
| 시험한 것 | 25가지 이상 · **채택 1** — 정교한 기법이 단순한 둘(200일 이평·균등가중)을 한 번도 이기지 못했다 |
...
| 조합 탐색 (팩터 20개 풀 → 폴드 내 상위 6개) | 16.90 → 15.83% | 아래 참조 |

> **단순한 쪽이 세 번 이겼다.** 200일 이평 하나가 낙폭을 −64%→−24%로 줄인 반면,
> 정교한 것들은 전부 성과를 떨어뜨렸다. DeMiguel et al.(2009)의 "1/N 을 이기기"가
> 실질 기준선이라는 결과가 우리 데이터에서 세 번 재현됐다.
...
> 개별 IC 는 팩터를 하나씩 세워놓고 재는 자다. **다른 팩터와 겹치지 않는 정보를
> 얼마나 더하는지**는 측정하지 않는다. 가치·품질 팩터는 서로 비슷한 종목을 고르고,
> 성장은 다른 축이다. IC 순으로 자르면 그 축이 통째로 사라진다.

선별 기준을 IC 가 아니라 **잔차 기여도**(이미 고른 팩터에 회귀하고 남은 부분의 IC)
로 바꾸는 것이 다음 후보다. 지금 결과로는 채택 전략을 대체하지 않는다.
