...
**현재 적재 상태** (2026-08-16 실측, 08-15 벌크 재구축 후):

| 테이블 | 행수 | 직전(08-15 이전) |
|---|---|---|
| `prices` | 45,663,897 | 46,331,751 |
| `actions` | 670,830 | 672,349 |
| `fundamentals` | 654,911 | 654,637 |
| `sp500` | 59,672 | 60,172 |
| `tickers` | 43,919 | 44,938 |
| `institutions` | **669,593** | 0 |
| `insiders` | 312,911 | 0 |
| `estimates` | 0 | 0 |

`institutions` 는 벌크 CSV 리더가 실제 헤더(`date`)가 아니라 `calendardate` 를
기대해 한 번도 동작한 적이 없었다 (2026-08-15 발견·수정, 이제 66.9만행).
`estimates` 는 Sharadar 에 애널리스트 추정치가 없어 **비어 있는 것이 정상**이다
(`00-overview.md` §2). "쓸 수 있다고 적혀 있는데 비어 있는 것"과 "설계상 없는
것"은 구분해서 적는다.

**`prices` 가 줄어든 것은 절단이 아니다.** 벤더가 SEP 벌크에서 워런트를 뺐다.

| | 08-12 벌크 | 08-15 벌크 |
|---|---|---|
| 종목 | 21,960 | 20,927 |
| 행 | 46,257,136 | 45,606,053 |
| 최신일 | 2026-08-10 | 2026-08-14 |
| 파일 크기 | 999,691,701 B | 988,694,275 B |

사라진 1,037종목의 벤더 `category` 는 `Domestic Common Stock Warrant` 917 ·
`ADR Common Stock Warrant` 115 · `Canadian ... Warrant` 3 — **1,035가 워런트**다.
나머지 2는 개명이며 신 벌크에 그대로 있다: `ARIA`→`ARIA1`(ARIAD, 상장
1994-09-19 · 폐지 2017-02-15 동일), `BNRG`→`BRNX`(Brenmiller→Brenx, 상장
2022-05-25 동일). 신규 상장 4종(`ADIG`·`WLCO` 등)이 더해져 순 −1,033이다.

성과 영향은 없다 — 워런트는 DAILY 가 시총을 주지 않아 유니버스 밴드에서 이미
100% 탈락하던 것들이고, 채택 유니버스 6,895종목 중 이번에 심볼이 바뀐 것은
`ARIA`·`BNRG` 둘뿐이다(유니버스 파일 갱신함).

> **4일치가 더 들어간 가격 파일이 1.1% 작아졌다** — 이것이 먼저 눈에 띈
> 신호였다. 행수·종목수 기대값 대조만으로는 "벤더가 줄인 것"과 "우리가
> 자른 것"을 구분할 수 없다. 판정은 항상 **원본 벌크와의 대조**로 한다.
...
**현재 구독 (2026-08 기준): Bundle · 풀 히스토리.** 실적재 결과는 20,931종목 /
