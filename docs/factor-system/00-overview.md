...
미국 주식 전종목(**실적재 21,963종목 / 1997~2026**, 상장폐지 포함)을 대상으로 한
...
> ⚠️ **슬리피지는 요청에 없었지만 기본 포함한다.** 수수료 0%로 소수 종목을 분기 리밸런싱하면
...
이 표는 구현 전에 작성됐고, 국내 플랫폼의 약어 정의가 공개 표준이 아니라
추정이 섞여 있었다.

**채택 전략이 쓰는 팩터들의 정의는 확정됐다** — 사용자의 실제 퀀터스 설정과
대조해 복제했고, 그 복제본이 walk-forward 를 통과했다. (어떤 조합인지는
공개하지 않는다. 이유는 `configs/README.md` 참조 — 초소형주 용량 문제다.)
아래에서 확신도 '낮음/중간'으로 남은 것들은 **채택 전략에 쓰이지 않는다** —
실험용 팩터이며, 쓰기 전에 정의를 다시 확인해야 한다.
