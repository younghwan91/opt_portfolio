...
> **이 문서는 설계 스펙이다.** 착수 시점의 계획 135개를 적었고, 실제 레지스트리는
> 실험 과정에서 늘어 **158개**다 (§7 참조). 현재 목록의 진실은 코드이며
...
이 문서는 **설계 스펙**이며, 위 표는 착수 시점의 계획이다. 이후 실험 과정에서
팩터가 추가돼 레지스트리는 158개다. 차이는 이렇게 생겼다:

| 파일 | 내용 |
|---|---|
| `library/experimental.py` | 직접 조립한 신규 팩터 10종 |
| `library/oap.py` | Chen & Zimmermann 오픈소스 자산가격 라이브러리 복제 12종 |
| `library/flow_proxy.py` | 13F·내부자 수급 프록시 5종 (§5 는 `00-overview.md`) |
