...
`uv run python scripts/run_taa.py` 실측 (2026-08-17):
...
baa_bal_tranche    0.0811 -0.0972 0.0816  0.8346  0.9998 0.9957 218.0000     0.5464
...
틀렸다 — 채택 기준인 Calmar 로 다시 보면 정반대다.
...
두 쌍 모두 이평이 Calmar 를 뚜렷이 올린다 — MDD 를 CAGR 보다 더 많이 깎기
때문이다(예: `baa_bal` MDD −11.1% → `baa_bal_ma` −8.3%, CAGR 은 7.3%→6.7% 로
소폭만 하락). 이중 필터가 VAA 의 병(과잉 방어)을 재발시킬 것이라는 예상은
빗나갔다.

**단, 트랜치 위에 얹으면 반대다.** `baa_bal_tranche`(0.835) → 이평을 더한
`baa_bal_ma_tranche`(0.731) 는 **내려간다.** 트랜치 재구현(§3) 이후의 최종
수치이며, 트랜치 재구현 이전 수치(0.97→1.26, Task 7 중간 로그)로 "셋 다
개선"이라 적으면 이미 폐기된 착시를 다시 쓰는 것이다. **이평 오버레이는
효과가 있지만 무조건은 아니다** — 이미 분산을 줄인 위에 또 줄이면 득이 아닐
수 있다.

### 예상 3 — 트랜치는 CAGR 을 거의 안 바꾸고 Calmar 를 조금 올린다. **대체로 맞음.**

트랜치 방법론 결함(§3)을 고친 뒤 다시 잰 슬리브 간 분산 감소 효과는 약 **4%**
(Task 7 fix round 2). 이 4%가 예상이 말한 "조금"에 해당한다. 결과표의
`baa_bal`(0.654) → `baa_bal_tranche`(0.835) Calmar 차이가 이보다 커 보이는
것은 트랜치가 리밸런싱 시점만 바꾸는 게 아니라(§4 스펙 결함들을 함께 흡수한
합성 실행이라) 단일 요인으로 못 나눈다 — **분산 감소 효과만 따로 잰 값(4%)이
예상 대조의 정답**이고, 표의 원시 Calmar 차이는 다른 요인이 섞인 값이다.
...
설계 문서(`2026-08-17-taa-strategy-design.md`)를 구현하며 다섯 가지 결함을
...
- 데이터는 2026-08-14 에서 멈춘다(Sharadar 구독 종료). 이후 갱신 없음.
