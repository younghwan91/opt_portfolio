...
- 데이터는 **풀 히스토리 번들**이다 — 21,963종목 / 1997~2026, 폐지 종목 포함
  (Enron·구 American Airlines·Ambac 실재 확인). **생존편향은 해소됐다.**
- 남은 제약은 다른 곳에 있다:
  - **용량.** 채택 전략의 유니버스는 초소형주다. 가치가중으로 바꾸면 Sharpe 가
    28% 깎인다 — 알파가 작은 종목에 있다는 뜻이고, 자금이 커지면 실현되지 않는다.
  - **거래비용 가정이 데이터가 아니다.** 수수료 0.5%·슬리피지 0 을 가정했고,
    이는 설계 문서가 필수로 적은 것을 끈 상태다 (§1-b). 검증 실행 중.
