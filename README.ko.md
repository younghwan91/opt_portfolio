...
| Deflated Sharpe (**전략 탐색 35회**) | **0.988** ✓ | 0.969 ✓ | — |
| PBO (35개 구성 CSCV) | **0.139** ✓ | — | — |

**이 전략은 방어 장치를 켠 채로 측정됐다** — 최소 주가 $5 · 최소 거래대금 $1M ·
슬리피지. 유니버스가 역대 S&P500 이라 **용량 제약이 없다.** 슬리피지를 50bps 로
올려도 낙폭과 변동성이 그대로고 수익만 1.4%p 깎인다.

마지막 두 줄이 2026-08-17 에 추가됐다. 그전까지 DSR 은 walk-forward **안쪽**의
파라미터 시도(72회)만 정산했고, *"35개 전략을 훑어 하나를 골랐다"* 는 상위 탐색은
계상되지 않았다 — 실험 기록이 부채로 남겨둔 항목이다. 35개 산출물을 공통 구간
(2009-12~, 201개월)으로 맞춰 다시 재도 **DSR 0.988 · PBO 0.139** 로 관문을 넘는다.
...
15년, 초기 $10,000 기준. Dynamic VAA 최종 자산 **$22,813**(CAGR 5.7%, Sharpe
0.110). 이전 버전 문서는 ~$29k 로 적었고 Sharpe 는 연율화 버그로 **20.9배**
부풀어 있었다(월별 수익률을 √252 로 연율화 — 정상은 √12, `b9043d7` 에서 수정).
둘 다 실측보다 낙관적이었다.

**CAGR 이 Keller 논문의 연 16~17% 대신 5.7% 인 이유:** 이 전략은 전체 기간의
**54.7% 를 방어 자산으로 보냈고**, 그중 `SHY` 한 종목에 **44개월(24.4%)** 동안
연 0.05% 수준(사실상 0)으로 묶였다 — 같은 기간 `SPY` 바이앤홀드는 훨씬 높았다.
Keller 의 표제 수치는 1970~2015 구간이며 그 시절 방어 자산 자체가 연 8~15%
를 줬다. 구현이 틀린 게 아니라 **전제(방어 자산의 시대별 수익률)가 지금과 안
맞는다.** 상세 진단: `docs/superpowers/specs/2026-08-17-taa-strategy-design.md` §0.
