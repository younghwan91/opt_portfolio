...
- **팩터 선택은 아직 정산되지 않았다.** 지금 쓰는 다섯 개는 124개를 훑어 고른
  것이고, 그 훑는 행위 자체가 탐색인데 어떤 시도 횟수에도 들어가 있지 않다.

> **갚은 부채 하나.** 상위 탐색 — 35개 전략을 돌려 하나를 고른 것 — 은 정산됐고
> 재현된다: 월별 수익률 · S=16 에서 **PBO 0.303 · DSR 0.982(15bps) / 0.957(50bps)**.
> 위 성과표의 마지막 두 줄이 그것이다. 왜 그 눈금인지와 그 전에 적었던 틀린 숫자
> 두 개는 위 *관문을 어느 눈금으로 재는가* 에 있다. 재현은
> `uv run python scripts/strategy_search_cost.py` — 구독 없이 돈다.
