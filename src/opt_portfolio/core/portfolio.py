...
class Portfolio:
    ...
    def __init__(self, holdings: dict[str, int] | None = None):
        """
    ...
    def calculate_rebalance(
        ...
        current_value = sum(
            self.positions[t].shares * prices.get(t, 0) for t in self.positions if t in prices
        )
        ...
        if target_allocations is None:
            ...
            if selected_etf in ["SPY", "TLT", "GLD", "BIL"]:
                ...
                target_allocations[selected_etf] = ALLOCATION.VAA_SELECTED_WEIGHT + core_weight

        # Calculate target values and shares
        target_values = {
            ticker: total_target_value * weight for ticker, weight in target_allocations.items()
        }
            ...
            if ticker not in target_allocations:
                ...
                if current_shares > 0 and ticker in prices:
                    ...
                    transactions[ticker] = Transaction(
                        ticker=ticker, action="SELL", shares=current_shares, price=prices[ticker]
                    )
            else:
        ...
        for item in shortfalls:
            ...
            if shares_to_buy > 0:
                ...
                if ticker in transactions:
                    ...
                    if net_shares > 0:
                        transactions[ticker] = Transaction(
                            ticker=ticker, action="BUY", shares=net_shares, price=item["price"]
                        )
                    elif net_shares < 0:
                    ...
                    transactions[ticker] = Transaction(
                        ticker=ticker, action="BUY", shares=shares_to_buy, price=item["price"]
                    )
        ...
        return {
            ...
            "optimization_summary": {
                ...
                "cash_utilization_rate": ((available_cash - remaining_cash) / available_cash * 100)
                if available_cash > 0
                else 0,
            },
        }
    ...
    def __repr__(self) -> str:
        ...
        positions_str = ", ".join(f"{t}: {p.shares}" for t, p in self.positions.items())
