...
from ..config import ASSETS, BACKTEST, MOMENTUM, AllocationConfig
...
class BacktestEngine:
    ...
    def run_dynamic_vaa_backtest(
        ...
        equity_series = pd.Series(
            equity_curve, index=[monthly_dates[0]] + dates_recorded[: len(equity_curve) - 1]
        )
        returns_series = pd.Series(monthly_returns, index=dates_recorded[: len(monthly_returns)])
    ...
    def run_optimized_backtest(
        ...
        result = self.run_dynamic_vaa_backtest(
            years=years, allocation_weights=opt_result.best_weights
        )

        return result, opt_result

    def _get_component_returns(self, years: int) -> tuple[pd.Series, pd.DataFrame]:
        """
    ...
    def run_vaa_backtest(
        ...
        for strategy in strategies:
            ...
            returns_series = pd.Series(
                monthly_returns[strategy], index=dates_recorded[: len(monthly_returns[strategy])]
            )
            ...
            result = BacktestResult(
                ...
                defensive_ratio=defensive_counts[strategy] / len(monthly_dates)
    ...
    def compare_with_benchmark(
        ...
        benchmark_data = self.cache.get_incremental_data([benchmark_ticker], start_date, end_date)
        ...
        information_ratio = (
            (strategy_annual - benchmark_annual) / tracking_error if tracking_error > 0 else 0
        )
