...
def create_equity_chart(
    equity_curves: dict[str, pd.Series], title: str = "Equity Curves", figsize: tuple = (12, 6)
) -> plt.Figure:
    """
...
def create_allocation_pie(
    allocations: dict[str, float], title: str = "Portfolio Allocation", figsize: tuple = (8, 8)
) -> plt.Figure:
    """
...
def create_drawdown_chart(
    equity_curve: pd.Series, title: str = "Drawdown", figsize: tuple = (12, 4)
) -> plt.Figure:
    """
...
def create_momentum_chart(
    ...
    for column in momentum_df.columns:
        ...
        ax.plot(momentum_df.index, momentum_df[column], label=column, color=color, linewidth=2)

        if forecast_df is not None and column in forecast_df.columns:
            ax.plot(forecast_df.index, forecast_df[column], linestyle="--", color=color, alpha=0.7)

    ax.axhline(y=0, color="black", linestyle="-", alpha=0.3)

    if not momentum_df.empty:
        ax.axvline(x=momentum_df.index[-1], color="gray", linestyle=":", alpha=0.5, label="Today")
...
def create_performance_summary(results: dict, figsize: tuple = (12, 8)) -> plt.Figure:
    """
