...
class SelectionResult:
    ...
    def __init__(
        ...
        strategy_recommendations: dict | None = None,
    ):
...
class VAAStrategy:
    ...
    def __init__(
        self,
        aggressive_tickers: list[str] | None = None,
        protective_tickers: list[str] | None = None,
        use_cache: bool = True,
        ...
        """
        self.aggressive_tickers = list(aggressive_tickers or ASSETS.AGGRESSIVE_TICKERS)
        self.protective_tickers = list(protective_tickers or ASSETS.PROTECTIVE_TICKERS)
    ...
    def select(
        self, calculation_date: date | None = None, strategy: StrategyType = StrategyType.CURRENT
    ) -> SelectionResult:
        """
        ...
        if self.use_forecasting and self.forecaster:
            strategy_recs = self._get_strategy_recommendations(calculation_date, is_defensive)

        # Select ETF based on strategy
        selected_etf = self._select_by_strategy(
            strategy, agg_ranked if not is_defensive else prot_ranked, strategy_recs, is_defensive
        )

        # Print results
        self._print_analysis(agg_ranked, prot_ranked, mode, selected_etf, strategy_recs)
    ...
    def _get_strategy_recommendations(self, calculation_date: date, is_defensive: bool) -> dict:
    ...
    def analyze_all_strategies(
        self, calculation_date: date | None = None
    ...
    def get_win_probabilities(
        self, calculation_date: date | None = None, months: int = 1
